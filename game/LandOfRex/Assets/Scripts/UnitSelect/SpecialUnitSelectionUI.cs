...
public class SpecialUnitSelectionUI : MonoBehaviour
{
    ...
    {
        // 각 버튼에 클릭 이벤트 등록
        magicianButton.onClick.AddListener(() => OnUnitSelected("Magician"));
        knightButton.onClick.AddListener(() => OnUnitSelected("Knight"));
        confirmButton.onClick.AddListener(OnConfirmSelection);

        // 초기 상태 설정
        OnUnitSelected("Magician"); // 첫 번째 버튼(전사)을 기본 선택으로
    }
